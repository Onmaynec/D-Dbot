...
def roll_initiative(
    ...
    order = [
        {"kind": "player", "name": "Герои", "initiative": state["player_initiative"]}
    ]
    for enemy in living_enemies(state):
        enemy["initiative"] = roller.randint(1, 20) + max(
            0,
            int(enemy["min_level"]) // 2,
        )
        order.append(
            {
                "kind": "enemy",
...
def cast_spell(
    ...
    if success:
        dice_count = 2 if critical else 1
        damage = sum(
            roller.randint(1, spell["damage_die"])
            for _ in range(dice_count)
        )
        damage += max(0, spell_modifier) + max(0, int(damage_bonus))
        damage = max(1, damage)
        target["hp"] = max(0, target["hp"] - damage)
        if target["hp"] == 0:
            target["alive"] = False
